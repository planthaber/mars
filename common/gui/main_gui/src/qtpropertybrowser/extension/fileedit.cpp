...
void FileEdit::buttonClicked()
{
  QString filePath;
  if (theFilter == "QDIR")
    filePath = QFileDialog::getExistingDirectory(this, tr("Choose a directory"), theDirectory);
  else
    filePath = QFileDialog::getOpenFileName(this, tr("Choose a file"), theDirectory, theFilter);
    if (filePath.isNull())
        return;
